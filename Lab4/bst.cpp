...
template <class Comparable>
int BinarySearchTree<Comparable>::isBalanced() const
{
	if(isEmpty())
	{
		cout << "Empty Tree" << endl;
...
template <class Comparable>
int BinarySearchTree<Comparable>::isBalanced(BinaryNode<Comparable> *t) const
{
	if(t!=NULL)
	{
		int left = height(t->left);
		int right = height(t->right);
		if(abs(left-right)<=1 && isBalanced(t->left) && isBalanced(t->right))
			return 1;
		else
			return 0;
	}
